...
export default function Dashboard({ 
    ...
    const stats = useMemo(() => [
        {
            ...
            value: `₹${parseFloat(todayRevenue || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
            icon: (
        ...
        },
        {
            ...
            changeType: servicesChange >= 0 ? 'increase' : 'decrease',
        ...
        },
        {
            ...
            changeType: purchasesChange >= 0 ? 'increase' : 'decrease',
    ...
    ], [todayRevenue, revenueChange, todayServices, servicesChange, todayPurchases, purchasesChange, totalCustomers, customersChange]);

    // Memoized recent activities with icons
    const activitiesWithIcons = useMemo(() => 
        (recentActivities || []).map((activity) => {
            const iconMap = {
                login: (
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12H3m0 0l4-4m-4 4l4 4m6 4h5a2 2 0 002-2V6a2 2 0 00-2-2h-5" />
                    </svg>
                ),
                logout: (
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H9m4 4H5a2 2 0 01-2-2V6a2 2 0 012-2h8" />
                    </svg>
                ),
                payment: (
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            return {
                ...activity,
                icon: iconMap[activity.type] || iconMap.create
            };
        }), 
        [recentActivities]
    );
                    ...
                    <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <div className="flex items-center justify-between">
                            <div className="text-sm text-gray-600 dark:text-gray-400">
                                Showing {activitiesWithIcons.length} recent activities from today
                            </div>
