...
export default function Edit({ subAdmin, menus, roles }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        selected_menus: subAdmin.menus?.map(m => m.id) || [],
        _method: 'PUT'
    });

    const [selectedRole, setSelectedRole] = useState(roles.find(r => r.id === subAdmin.role_id));

    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('sub-admins.update', subAdmin.id));
    };
    ...
    return (
        <SidebarLayout>
            <Head title={`Edit Sub-Admin - ${subAdmin.name}`} />
            <div className="p-6">
                <div className="mb-6">
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Edit Sub-Admin</h1>
                    <p className="text-gray-600 dark:text-gray-400 mt-2">Update sub-admin account details and menu access.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm p-6">
                    ...
                    <div className="flex justify-end gap-3">
                        ...
                        >
                            {processing ? 'Updating...' : 'Update Sub-Admin'}
                        </button>
                    </div>
                </form>
            </div>
